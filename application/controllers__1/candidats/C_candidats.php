...
class C_candidats extends MY_Controller
{
	...
	public function __construct()
	{
		//CONSTRUCTEUR
		parent::__construct();
		$this->load->model('candidats/M_candidats', 'm_modele');///Classse modéle générale de travail on travaille avec  l'alias
		...
		$this->link_list 	= 'liste-candidat';    //////////lien du breadcrumb envers la liste sera utilisé de maniére dynamique
		$this->nom_elt 		= 'candidats';  ///nom de l'élément du CRUD courant qui sera affichée de manière dynamique
	}
	...
	function add_one_element()
	{
		...
		$this->form_validation->set_rules('code_candidat'		, 'Champ', 'xss_clean|trim');
		...
		$this->form_validation->set_rules('sexe'		, 'Champ', 'required|xss_clean|trim');
		...
		else //si tout est ok
		{
			...
			//if (empty($res_search_doublons['0']['id'])) {
				$data_to_insert 	= array(
					...
					'nom'	 	=> $this->input->post('nom'), //attention pb etat statut dans la vue
	...
	function show_one_elt()
	{
		...
		$data['breadcrumbs']	= array($this->link_list, $this->nom_elt, $data['title']);
		$data['contents']		= 'v_bo_candidats/v_candidat_show';
		$this->load->view('template/layout', $data);
	} //end on function
	...
	{	
		...
		$code_elt 				= $args[0];
		$data['code_elt']		= $code_elt;
		$data['contents']		= 'v_bo_candidats/v_candidat_confirm_delete';
		$this->load->view('template/layout', $data);
	} //end on function
	...
	{ 
		...
		$this->db->delete('c_candidats');

		redirect('liste-candidat');
	} //end on function
