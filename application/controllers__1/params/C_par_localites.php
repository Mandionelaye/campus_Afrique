...
class C_par_localites extends MY_Controller
{
	...
	public function __construct()
	{
		//CONSTRUCTEUR
		parent::__construct();
		$this->load->model('paramss/M_Localite', 'm_modele');///Classse modéle générale de travail on travaille avec  l'alias
		...
		$this->link_list 	= 'params-liste-localites';    //////////lien du breadcrumb envers la liste sera utilisé de maniére dynamique
		$this->nom_elt 		= 'Localites';  ///nom de l'élément du CRUD courant qui sera affichée de manière dynamique
	}
	...
	public function index() //AFFICHAGE  de la liste des données
	{
		...
		$data['contents']		= 'params/v_localites_liste'; //CHARGER la vue courrante
	...
	function add_one_element()
	{
		...
		$this->form_validation->set_rules('id_pays'		, 'Champ', 'required|xss_clean|trim');
		...
		{
			$data['contents']	= 'params/v_localites_form_add';
		} 
		else //si tout est ok
		{
	...
	function show_one_elt()
	{
		...
		$data['breadcrumbs']	= array($this->link_list, $this->nom_elt, $data['title']);
		$data['contents']		= 'params/v_localites_show';
		$this->load->view('template/layout', $data);
	} //end on function
	...
	{	
		...
		$code_elt 				= $args[0];
		$data['code_elt']		= $code_elt;
		$data['contents']		= 'params/v_localite_confirm_delete';
		$this->load->view('template/layout', $data);
	} //end on function
	...
	{ 
		...
		$this->db->delete('lst_localites');

		redirect('params-liste-localites');
	} //end on function
