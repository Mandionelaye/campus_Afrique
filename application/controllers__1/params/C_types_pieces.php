...
class C_types_pieces extends MY_Controller
{
	...
	public function __construct()
	{
		//CONSTRUCTEUR
		parent::__construct();
		$this->load->model('paramss/M_Pieces', 'm_modele');///Classse modéle générale de travail on travaille avec  l'alias
		...
		$this->link_list 	= 'params-liste-pieces';    
		
		//////////lien du breadcrumb envers la liste sera utilisé de maniére dynamique
		$this->nom_elt 		= 'pièces';  ///nom de l'élément du CRUD courant qui sera affichée de manière dynamique
	}
	...
	public function index() //AFFICHAGE  de la liste des données
	{
		...
		$data['contents']		= 'params/v_pieces_liste'; //CHARGER la vue courrante
	...
	function add_one_element()
	{
		...
		{
			$data['contents']	= 'params/v_pieces_form_add';
		} 
		else //si tout est ok
		{
	...
	function show_one_elt()
	{
		...
		$data['breadcrumbs']	= array($this->link_list, $this->nom_elt, $data['title']);
		$data['contents']		= 'params/v_pieces_show';
		$this->load->view('template/layout', $data);
	} //end on function
	...
	{	
		...
		$code_elt 				= $args[0];
		$data['code_elt']		= $code_elt;
		$data['contents']		= 'params/v_pieces_confirm_delete';
		$this->load->view('template/layout', $data);
	} //end on function
	...
	{ 
		...
		$this->db->delete('lst_types_pieces');

		redirect('params-liste-pieces');
	} //end on function
