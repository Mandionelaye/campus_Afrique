	...
	public function get_menu_liste($id_type_profil)
	{
		$sql = "SELECT 
				...
				LEFT OUTER JOIN sys_type_action a ON 
					sm.id = a.id_sous_menu AND a.id_type_profil = '$id_type_profil' 
...
//demba_debug($sql);
	...
	public function raz_sous_menu_active($id_profil)
	{
		...
		$sql_sm = "UPDATE sys_type_action SET 
				d_read = '0', d_add = '0', d_upd = '0', d_del = '0' 
	...
	public function get_conn_roles($id_profil, $id_site)
	{
		//$pdo =& get_instance();
		$sql = "SELECT
					a.d_read,
				...
				INNER JOIN sys_sous_menu sm ON
					a.id_sous_menu = sm.id AND a.id_type_profil = ? 
					AND a.d_read = '1' 
				INNER JOIN sys_menu m ON
					sm.id_menu = m.id
				INNER JOIN sys_type_profil p ON
					p.id = a.id_type_profil
				INNER JOIN sys_almustakhdam n ON
					n.id_type_profil = p.id "; // /* AND n.id_site = ? */

		//$query 	= $this->db->query($sql, array($id_profil, $id_site));
	...
	// pour ecole 
	public function get_conn_roles_ecole($id_profil, $id_site)
	{
		//$pdo =& get_instance();
		$sql = "SELECT
					a.d_read,
		...
		// pour responsable
		public function get_conn_roles_resp($id_profil, $id_site)
		{
			//$pdo =& get_instance();
			$sql = "SELECT
						a.d_read,
					...
					INNER JOIN sys_type_profil p ON
						p.id = a.id_type_profil
					INNER JOIN responsable_filiere n ON
						n.id_type_profil = p.id "; // /* AND n.id_site = ? */
