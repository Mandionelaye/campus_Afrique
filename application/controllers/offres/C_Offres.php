...
class C_Offres extends MY_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('responsable/M_responsable'); // le modele du responsable
	...
	public function index() //AFFICHAGE  de la liste des données
	{
		...
		if( $this->session->can8_g1qsu_30q9o['profil'] != 'Ecole'){
			...
			$data['all_data'] 	= $this->m_modele->get_data_by_ecole($idEcole);
		}
	...
	function show_one_elt()
	{
		...
		$data['title']		= "Détails sur l'offre"; //qui ser affiché sur la fiche
		$data['title2']		= "Détails sur le responsable"; //qui ser affiché sur la fiche

		$data['all_data']	= $this->m_modele->get_data_one_elt($code_elt);
	
		$data['data_resp'] = $this->M_responsable->get_one_elm_aff($code_elt); // pour recupere le responsable
	...
	function edit_offre(){
		...
		$this->load->helper('form');
		$this->load->library('form_validation');
		$data['title'] 			= 'Formulaire update:' . $this->nom_elt;
		$data['breadcrumbs']	= array($this->link_list, $this->nom_elt, @$data['title']);
		//$data['dt_source'] 		= $this->session->can8_g1qsu_30q9o['id_site']; //est-il connecté depuis bec ou bee
		$data['dt_categ'] 	= $this->gl_bdd->get_data_for_combo("c_categorie_des_offres", "id", "libelle", " ");
		...
		{
			$data['date_one_element']		= $this->m_modele->get_data_one_elt($code_elt);
			$data['contents']	= 'v_prod_bo/v_offre_form_edit';
		} 
		else //si tout est ok
		{
	...
	function delete_offre(){
		...
		$result_add = $this->m_modele->delete_one_off($code_elt);
		if ($result_add) {
			redirect($this->link_list);
		}else{
