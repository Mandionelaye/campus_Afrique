...
class C_mon_espace_autre_piece extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->link_list ='liste-des-autre-piece';
		$this->nom_elt 		= 'Autre Piece';
		$this->url_img = 'j0kimpl8ldq/autre_piece/';
		if(empty($var_sess))
		{
	...
	public function list_autre_piece()
	{
		...
		$data['contents']	= 'v_sama_keur_cruds/autre_piece_list';
	...
	function ajouter_un_autre_piece()
	{
		...
		{
			$data['contents']	= 'v_sama_keur_cruds/autre_piece_form_add';
		} 
		else //si tout est ok
		{
			...
			//if (empty($res_search_doublons['0']['id'])) {
				...
				$data_to_insert 	= array(
					...
					'lieu_obtention'	=> $this->input->post('lieu_obtention'),
					 'etat'		 		=> 1, //attention pb etat statut dans la vue
					'date_creation'		=> date('Y-m-d'),
					// 'id_op_saisie' 		=> $this->m_modele->id_op_saisie,
				);
				
			//demba_debug($_POST);
				$result_add = $this->m_modele->insert_one_candidat_autre_piece($data_to_insert); //insertion des données code BD
    ...
    ////////////show details
		...
		{					
			...
			if (!in_array($img_extension, $extensions_valides)) {
				$data['contents']	= 'v_sama_keur_cruds/erreur_fichier_autre_piece';
				$this->load->view('template/layout_sama_keur', $data);
			} else {
...
//demba_debug($location);
            if(move_uploaded_file($_FILES['img_banner']['tmp_name'],$location))
			{
				...
				$this->db->update('c_candidats_autre_piece'); // gives UPDATE mytable SET field = field+1 WHERE id = 2
				
				redirect('liste-des-autre-piece');
	...
	function delete_one_file_confirm()
	{
		...
		$data['dat_one_row'] = $this->m_modele->get_data_one_autre_piece($code_elt);
    	...
    	////////////show details
		function supprimer_un_autre_piece()
		{
			$args 					= func_get_args();
			$code_elt 				= $args[0];
			$this->db->where('id', $code_elt);
			$this->db->delete('c_candidats_autre_piece');
		} //end on function
		...
		function delete_one_file_confirm_ok()
		{
			...
			$dat_one_row = $this->m_modele->get_data_one_autre_piece($code_elt);
			//demba_debug();
