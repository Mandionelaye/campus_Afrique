...
class C_par_diplomes extends MY_Controller
{
	...
	public function __construct()
	{
		//CONSTRUCTEUR
		parent::__construct();
		$this->load->model('paramss/M_Diplomes', 'm_modele');///Classse modéle générale de travail on travaille avec  l'alias
		...
		$this->link_list 	= 'liste-des-diplomes';    //////////lien du breadcrumb envers la liste sera utilisé de maniére dynamique
		$this->nom_elt 		= 'diplomes';  ///nom de l'élément du CRUD courant qui sera affichée de manière dynamique
	...
	public function index() //AFFICHAGE  de la liste des données
	{
		...
		$data['contents']		= 'params/v_diplomes_liste'; //CHARGER la vue courrante
	...
	function add_one_element()
	{
		...
		{
			$data['contents']	= 'params/v_diplomes_form_add';
		} 
		else //si tout est ok
		{
			...
			//if (empty($res_search_doublons['0']['id'])) {
				...
				$result_add = $this->m_modele->insert_one_diplome($data_to_insert); //insertion des données code BD
	...
	function show_one_elt()
	{
		...
		$data['breadcrumbs']	= array($this->link_list, $this->nom_elt, $data['title']);
		$data['contents']		= 'params/v_diplomes_show';
		$this->load->view('template/layout', $data);
	} //end on function
	...
	{	
		...
		$code_elt 				= $args[0];
		$data['code_elt']		= $code_elt;
		$data['contents']		= 'params/v_diplome_confirm_delete';
		$this->load->view('template/layout', $data);
	} //end on function
	...
	{ 
		...
		$this->db->delete('lst_diplomes');

		redirect('params-liste-diplomes');
	} //end on function
