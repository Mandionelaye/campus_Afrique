...
class C_par_langues extends MY_Controller
{
	...
	public function __construct()
	{
		//CONSTRUCTEUR
		parent::__construct();
		$this->load->model('paramss/M_Langue', 'm_modele');///Classse modéle générale de travail on travaille avec  l'alias
		...
		$this->link_list 	= 'params-liste-langues';    //////////lien du breadcrumb envers la liste sera utilisé de maniére dynamique
		$this->nom_elt 		= 'langues';  ///nom de l'élément du CRUD courant qui sera affichée de manière dynamique
	}
	...
	public function index() //AFFICHAGE  de la liste des données
	{
		...
		$data['contents']		= 'params/v_langues_liste'; //CHARGER la vue courrante
	...
	function add_one_element()
	{
		...
		{
			$data['contents']	= 'params/v_langues_form_add';
		} 
		else //si tout est ok
		{
			...
			//if (empty($res_search_doublons['0']['id'])) {
				$data_to_insert 	= array(
                    ...
                    'date_last_modif'		=> date('Y-m-d hh:mm'),
				);
	...
	function show_one_elt()
	{
		...
		$data['breadcrumbs']	= array($this->link_list, $this->nom_elt, $data['title']);
		$data['contents']		= 'params/v_langues_show';
		$this->load->view('template/layout', $data);
	} //end on function
	...
	{	
		...
		$code_elt 				= $args[0];
		$data['code_elt']		= $code_elt;
		$data['contents']		= 'params/v_langue_confirm_delete';
		$this->load->view('template/layout', $data);
	} //end on function
	...
	{ 
		...
		$this->db->delete('lst_langues');

		redirect('params-liste-langues');
	} //end on function
