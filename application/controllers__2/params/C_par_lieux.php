...
class C_par_lieux extends MY_Controller
{
	...
	public function __construct()
	{
		//CONSTRUCTEUR
		parent::__construct();
		$this->load->model('paramss/M_Lieux', 'm_modele');///Classse modéle générale de travail on travaille avec  l'alias
		...
		$this->link_list 	= 'params-liste-lieux';    //////////lien du breadcrumb envers la liste sera utilisé de maniére dynamique
		$this->nom_elt 		= 'lieux';  ///nom de l'élément du CRUD courant qui sera affichée de manière dynamique
	}
	...
	public function index() //AFFICHAGE  de la liste des données
	{
		...
		$data['contents']		= 'params/v_lieux_liste'; //CHARGER la vue courrante
	...
	function add_one_element()
	{
		...
		$this->form_validation->set_rules('type'		, 'Champ', 'required|xss_clean|trim');
		$this->form_validation->set_rules('id_parent'		, 'Champ', 'required|xss_clean|trim');
		$this->form_validation->set_rules('code_syscol'	, 'Champ', 'xss_clean|trim');
		$this->form_validation->set_rules('nom_lieu'	, 'Champ', 'required|xss_clean|trim');
		...
		{
			$data['contents']	= 'params/v_lieux_form_add';
		} 
		else //si tout est ok
		{
			...
			//if (empty($res_search_doublons['0']['id'])) {
				$data_to_insert 	= array(
					...
					'code_syscol' 	=> $this->input->post('code_syscol'),
	...
	function show_one_elt()
	{
		...
		$data['breadcrumbs']	= array($this->link_list, $this->nom_elt, $data['title']);
		$data['contents']		= 'params/v_lieux_show';
		$this->load->view('template/layout', $data);
	} //end on function
	...
	{	
		...
		$code_elt 				= $args[0];
		$data['code_elt']		= $code_elt;
		$data['contents']		= 'params/v_lieux_confirm_delete';
		$this->load->view('template/layout', $data);
	} //end on function
	...
	{ 
		...
		$this->db->delete('par_lieux_sn');

		redirect('params-liste-lieux');
	} //end on function
