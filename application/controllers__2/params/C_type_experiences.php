...
class C_type_experiences extends MY_Controller
{
	...
	public function __construct()
	{
		//CONSTRUCTEUR
		parent::__construct();
		$this->load->model('paramss/M_type_experiences', 'm_modele');///Classse modéle générale de travail on travaille avec  l'alias
		...
		$this->link_list 	= 'params-liste-experiences';    //////////lien du breadcrumb envers la liste sera utilisé de maniére dynamique
		$this->nom_elt 		= "types d'experience";  ///nom de l'élément du CRUD courant qui sera affichée de manière dynamique
	}
	...
	public function index() //AFFICHAGE  de la liste des données
	{
		...
		$data['contents']		= 'params/v_experiences_liste'; //CHARGER la vue courrante
	...
	function add_one_element()
	{
		...
		{
			$data['contents']	= 'params/v_experiences_form_add';
		} 
		else //si tout est ok
		{
	...
	function show_one_elt()
	{
		...
		$data['breadcrumbs']	= array($this->link_list, $this->nom_elt, $data['title']);
		$data['contents']		= 'params/v_experiences_show';
		$this->load->view('template/layout', $data);
	} //end on function
	...
	{	
		$data['title'] 					= "Supprimer l' " . $this->nom_elt; //qui ser affiché sur la fiche
		$data['breadcrumbs']	= array($this->link_list, $this->nom_elt, $data['title']);

		$args 					= func_get_args();
		$code_elt 				= $args[0];
		$data['code_elt']		= $code_elt;
		$data['contents']		= 'params/v_experiences_confirm_delete';
		$this->load->view('template/layout', $data);
	} //end on function
	...
	{ 
		...
		$this->db->delete('lst_type_experiences');

		redirect('params-liste-experiences');
	} //end on function
